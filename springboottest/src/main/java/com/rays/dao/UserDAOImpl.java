...
public class UserDAOImpl extends BaseDAOImpl<UserDTO> implements UserDAOInt {
	...
	public void delete(UserDTO dto, UserContext userContext) {
		...
		if (id != null) {
	        AttachmentDTO attachmentDto = attachmentService.findByPK(id, userContext); // Build code
	        if (attachmentDto != null) {
	            attachmentService.delete(attachmentDto, userContext); // Build code
	        } else {
	            System.out.println("No attachment found for Image ID: " + id);
	        }
	    } else {
	        System.out.println("Image ID is null for UserDTO with ID: " + dto.getId());
	    }
	}	
